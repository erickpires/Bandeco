...
public abstract class BaseAdapterDecorator extends BaseAdapter implements SectionIndexer, DynamicListView.Swappable {
	...
	/**
	 * If the adapter's list-erick.bandeco.view is hosted inside a parent(/grand-parent/etc) that can scroll horizontally, horizontal swipes won't
	 * work, because the parent will prevent touch-events from reaching the list-erick.bandeco.view.
	 *
	 ...
	 * work, because the parent will prevent touch-events from reaching the list-erick.bandeco.view.
	 *
	 * If a list-item erick.bandeco.view has a child with the given resource-ID, the user can still swipe the list-item by touching that child.
	 * If the user touches an area outside that child (but inside the list-item erick.bandeco.view), then the swipe will not happen and the parent
